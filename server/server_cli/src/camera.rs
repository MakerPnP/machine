...
use ergot::{NetStackSendError, topic};
...
pub async fn camera_streamer(
    ...
    loop {
        ...
        trace!(
            "Sending frame, now: {:?}, frame_number: {}, total_chunks: {}, len: {}",
            now, camera_frame.frame_number, total_chunks, total_bytes
        );
        ...
        if stack
            .topics()
            .broadcast::<CameraFrameChunkTopic>(&frame_chunk, topic_name)
            .is_err()
        {
        ...
        let mut ok = true;
        for (chunk_index, chunk) in jpeg_bytes
            .chunks(chunk_size)
            .enumerate()
        {
            let frame_chunk = CameraFrameChunk {
            ...
            match result {
                ...
                Err(e) => {
                    error!(
                        "Aborting frame, error sending chunk. frame_number: {}, chunk: {}/{}, retries: {}, error: {:?}",
                        frame_number,
                        chunk_index + 1,
                        total_chunks,
                        retries,
